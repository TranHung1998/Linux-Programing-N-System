...
static ssize_t pcd_write(struct file *filp, char __user *buff, size_t count, loff_t *f_pos);
...
static ssize_t pcd_read(struct file *filp, char __user *buff, size_t count, loff_t *f_pos)
{
    ...
    *(iomodule_drv.mem_addr->output_val) = ioread32(iomodule_drv.dev_ioaddr);
    ...
    memcpy(kernel_buff,iomodule_drv.mem_addr->output_val,REG_MAX_SIZE);
    pr_info ("User buff: %s\n", buff);
...
static ssize_t pcd_write(struct file *filp, char __user *buff, size_t count, loff_t *f_pos)
{
    ...
    pr_info("Read request of %zu bytes. Current file position: %lld\n",count,*f_pos);
...
static int __init read_write_IOmodule_init(void)
{
    ...
    iomodule_drv.mem_addr->output_val = iomodule_drv.mem_addr->input_val + REG_MAX_SIZE;
